...
function RadioCheckbox({
  ...
  return (
    <label htmlFor={id} className="flex items-center gap-2 cursor-pointer">
      ...
      <div className="flex items-center gap-[6px]" onClick={onChange}>
        <div
...
export default function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ==========================================================
      // Substitua o bloco abaixo pela chamada à sua API/banco.
      //
      // Os dados do formulário estão disponíveis em:
      //   form.nome      → string
      //   form.email     → string
      //   form.telefone  → string (opcional)
      //   form.empresa   → string (opcional)
      //   form.assunto   → string (mensagem)
      //   selected       → string[] (ids dos serviços marcados)
      //                    ex: ["landing-page", "e-commerces"]
      //
      // ── Opção 1: REST API própria ──────────────────────────
      ...
      //   nome: form.nome, email: form.email, mensagem: form.assunto,
      // }, "PUBLIC_KEY");
      //
      ...
      //   ...form, servicos: selected, criadoEm: new Date(),
      // });
      //
      // ── Opção 4: Supabase (PostgreSQL) ─────────────────────
      // import { supabase } from "@/lib/supabase";
      // const { error } = await supabase.from("contatos").insert([{
      //   nome: form.nome, email: form.email,
      //   telefone: form.telefone, empresa: form.empresa,
      //   servicos: selected, mensagem: form.assunto,
      // }]);
      // if (error) throw error;
      //
      // ── Opção 5: Nodemailer via Next.js API Route ──────────
      // Crie pages/api/contato.ts (ou app/api/contato/route.ts)
      // e faça fetch para "/api/contato" como na Opção 1.
      // ==========================================================

      // Simulação temporária — remova quando integrar o banco
        ...
        <div className="w-full md:flex-1 md:max-w-[520px]">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 md:gap-4" noValidate>
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
              ...
              <div className="flex flex-col gap-1 flex-1">
                <label style={labelBase}>Nome Da Empresa</label>
                <input
                  type="text"
            ...
            <div className="flex flex-col gap-2 md:gap-3">
              <label style={labelBase}>Assunto</label>
            ...
            <div className="flex flex-col gap-1">
              <label style={labelBase}>
                Assunto <span style={{ color: "#e30613" }}>*</span>
              </label>
              <textarea
